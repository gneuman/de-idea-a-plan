---
name: to-linear
description: Toma el plan generado por /idea (o cualquier spec en la conversación) y crea sus milestones + issues en Linear. Cada milestone de el plan se vuelve un Project Milestone nativo de Linear, y cada pieza de trabajo un issue colgado dentro. Use when "manda a Linear", "crea los issues", "sube el plan a Linear", "/to-linear".
user_invocable: true
...
Convierte un plan (el que generó `/idea`, en `_plan/plan.md`) en issues + Project Milestones de Linear. Es el puente entre "tengo el plan" y "el equipo ya lo puede tomar".
...
- Si existe `_plan/plan.md` en el repo, léelo. Es tu fuente de milestones y scope.
...
Rompe cada milestone de el plan en issues **independientemente tomables**. Prefiere **vertical slices** (tracer bullets): cada issue entrega un camino completo aunque angosto por todas las capas (datos, API, UI), NO una tajada horizontal de una sola capa.
...
Cada milestone de el plan = un Project Milestone nativo de Linear; los issues viven dentro.

1. Detecta los milestones de el plan (headings `## Milestone N — <nombre>` en `plan.md`).
2. `list_milestones` en el proyecto. Para cada milestone que NO exista, créalo con `save_milestone` (name = nombre del milestone; description = el framing del milestone de el plan).
...
El patrón de vertical slices / tracer bullets viene de [mattpocock/skills/engineering/to-tickets](https://github.com/mattpocock/skills/tree/main/skills/engineering/to-tickets) (MIT). Este fork lo adapta para arrancar desde el plan de `/idea` y publicar en Project Milestones nativos de Linear.

## [CUSTOMIZE]

```yaml
espec_path: _plan/plan.md          # de dónde lee el plan
